...
pub(super) async fn install<R: Runtime>(
    ...
    if needs_loader_manifest_download(&json_path) {
        ...
        let info = SimpleVersionInfo {
            ...
            loader_type: lighty_loaders::types::Loader::Fabric,
        };

        use lighty_loaders::loaders::fabric::fabric::FabricQuery;
        use lighty_loaders::utils::query::Query;
        let raw_metadata: <FabricQuery as Query>::Raw = serde_json::from_str(&raw_json_text)?;
        let merged_version = FabricQuery::version_builder(&info, &raw_metadata).await.map_err(|e| AppError::Generic(e.to_string()))?;
